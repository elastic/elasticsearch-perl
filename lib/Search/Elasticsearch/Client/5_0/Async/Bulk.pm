...
#===================================
    ...
    if ( ref $src eq 'HASH' ) {
        ...
        && $src->isa('Search::Elasticsearch::Client::5_0::Scroll') )
    {
        my $scroll = $src;
        $src = sub {
    ...
    # async scroll
    if ( blessed($src)
        && $src->isa('Search::Elasticsearch::Client::5_0::Async::Scroll') )
    {
        $promise = $src->start;
    }
    else {
...
L<Search::Elasticsearch::Client::5_0::Async::Scroll/new()> to create a new scrolled search.
