...
do "$FindBin::RealBin/parse_spec_base_8.pl" || die $@;
...
my $pathSpecs = sprintf("%s/util/rest-spec/%s/rest-api-spec/api", $dirname, $version);
...
process_files( 
    sprintf("%s/lib/Search/Elasticsearch/Client/8_0/Role/API.pm", $dirname),
    @files
);
